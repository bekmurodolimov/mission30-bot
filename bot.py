...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tarif_key not in TARIFLAR:
        await update.message.reply_text(
            "Salom! 👋 Kitob sotib olish uchun saytdagi tugmani bosing."
        )
    ...
    await update.message.reply_text(
        ...
        f"💳 <code>{KARTA}</code>\n\n"
        f"Pul o'tkazgandan so'ng <b>chek rasmini</b> shu yerga yuboring — "
...
async def chek_qabul(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Tekshirib, kitobni tez orada yuboraman. "
        "Odatda 5–30 daqiqa ichida yetkaziladi 📚"
    )
...
async def notogri(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
